...
library(dplyr)
     ...
     dptosUsar = c("CONTRAFUE-CASCO","CORTE","CORTE Y PREPARA","CUÑA","ENSAMBLES",
...
nuestro análisis tiene una media de **`r round(mean(sd.plot[sd.plot$FUNCIONCOMUN == "PESPUNTADOR",]$PERSONAS),2)`**
...
grado, al tener un correlación de **`r round(cor(p.plot$R.PESP, p.plot$R.PREL),2)`**.  
...
Como se puede ver en la siguiente gráfica, generar **`r familias`** familias parece lo más adecuado, pues a partir del  
...
El análisis se realiza definiendo **`r familias`** clusters, en este caso **`r familias`** familias.
...
Al iniciar el análisis vimos que el promedio de pespuntadores para producir 100 pares por hora  era `r round(mean(sd.plot[sd.plot$FUNCIONCOMUN == "PESPUNTADOR",]$PERSONAS),2)`  
, con una desviación estandar de `r round(sd(sd.plot[sd.plot$FUNCIONCOMUN == "PESPUNTADOR",]$PERSONAS),2)`.
